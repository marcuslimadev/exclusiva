...
const criarMarker = (lat, lng, imovel) => {
  const isVenda = imovel.finalidade_imovel === 'Venda'
  ...
  const iconHtml = `
    <div class="custom-marker-icon" style="background: ${gradient}">
      <i class="fas fa-${isVenda ? 'tag' : 'key'}"></i>
      <div class="marker-badge" style="background: ${isVenda ? '#f59e0b' : '#ec4899'}">
        ${isVenda ? 'V' : 'A'}
      </div>
    </div>
  `
...
const criarPopupContent = (imovel) => {
  const preco = formatarMoeda(imovel.valor_venda || imovel.valor_aluguel)
  ...
  return `
    <article class="popup-content">
      <figure class="popup-image">
        ...
        <span class="popup-badge ${imovel.finalidade_imovel === 'Venda' ? 'badge-venda' : 'badge-aluguel'}">
          ${imovel.finalidade_imovel}
        </span>
      </figure>
      ...
      <div class="popup-body">
        ...
        <button 
          onclick="window.dispatchEvent(new CustomEvent('open-property-details', {detail: ${imovel.codigo_imovel}}))"
          class="popup-button">
...
onMounted(async () => {
  ...
  window.addEventListener('open-property-details', (e) => {
    const imovel = props.imoveis.find(i => i.codigo_imovel === e.detail)
    if (imovel) {
      emit('property-click', imovel)
    }
  })
